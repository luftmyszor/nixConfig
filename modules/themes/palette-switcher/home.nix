    ...
    text = ''
      ...
      CSS_EOF
            ...
            render_zen() {
              ...
              if [[ ! -d "$zen_dir" ]]; then
                log "Zen browser profile directory not found: $zen_dir – skipping"
                return 0
              fi
              local rendered=0
              for profile_dir in "$zen_dir"/*/; do
                ...
                mkdir -p "$chrome_dir"
                cat > "$chrome_dir/zen-palette.css" <<CSS_EOF
      /* Generated by palette-switch – do not edit by hand */

      :root {
        --zen-primary-color: $primary;

        /* Palette custom properties – available to all zen/userChrome CSS */
        --palette-bg:        $bg;
        --palette-program-bg: $programBg;
        --palette-fg:        $fg;
        ...
        /* Firefox / Zen browser chrome overrides */
        --toolbar-bgcolor:              $bg !important;
        --tab-selected-bgcolor:         $dark !important;
        --toolbarbutton-icon-fill:      $fg !important;
        --urlbar-box-bgcolor:           $programBg !important;
        --urlbar-box-focus-bgcolor:     $dark !important;
        --urlbar-box-hover-bgcolor:     $dark !important;
        --urlbar-box-active-bgcolor:    $dark !important;
        --urlbar-popup-url-color:       $info !important;
        --urlbar-popup-action-color:    $secondary !important;
        --sidebar-background-color:     $programBg !important;
        --zen-apzoom-color:             $primary !important;
      }
      CSS_EOF
                log "Rendered Zen palette CSS → $chrome_dir/zen-palette.css"
                rendered=$((rendered + 1))
              done
              if [[ $rendered -eq 0 ]]; then
                log "No Zen browser profiles found in $zen_dir – skipping"
              fi
            }
            ...
            reload_zen() {
              # Zen browser picks up userChrome.css changes on next window open;
              # no signal is needed for the CSS custom-property approach.
              log "Zen palette CSS written – changes take effect on next Zen window"
            }
