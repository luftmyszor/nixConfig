  ...
  paletteSwitchScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      CSS_EOF
            ...
            render_zen() {
              load_palette
              # Write userChrome.css to all Zen profile locations that exist.
              # Handles both ~/.zen/ (legacy) and ~/.config/zen/ (XDG) profile roots
              # per https://docs.zen-browser.app/guides/live-editing
              local rendered=0
              for zen_root in "$HOME/.zen" "$HOME/.config/zen"; do
                local chrome_dir="$zen_root/default/chrome"
                # Only write to a root where the default profile directory already exists.
                [[ -d "$zen_root/default" ]] || continue
                mkdir -p "$chrome_dir"
      ...
      :root {
        --zen-palette-bg:        $bg;
        --zen-palette-programBg: $programBg;
        --zen-palette-fg:        $fg;
        --zen-palette-primary:   $primary;
        --zen-palette-secondary: $secondary;
        --zen-palette-tertiary:  $tertiary;
        --zen-palette-accent:    $accent;
        --zen-palette-muted:     $muted;
        --zen-palette-danger:    $danger;
        --zen-palette-success:   $success;
        --zen-palette-warning:   $warning;
        --zen-palette-info:      $info;
        --zen-palette-dark:      $dark;
        --zen-palette-light:     $light;
        --zen-palette-white:     $white;
      }
      ...
      :root {
        ...
        --toolbarbutton-hover-background:  $muted !important;
        --toolbarbutton-active-background: $dark !important;
      }

      /* URL bar */
      #urlbar {
        background-color: $programBg !important;
        color:            $fg !important;
      ...
      CSS_EOF
                log "Rendered Zen Browser CSS → $chrome_dir/userChrome.css"
                rendered=$((rendered + 1))
              done
              if [[ $rendered -eq 0 ]]; then
                log "No Zen profile directory found yet – CSS will be written on next activation"
              fi
            }
            ...
            apply_all() {
              ...
              render_zen       || log_err "Zen render failed"
